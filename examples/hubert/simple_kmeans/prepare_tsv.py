# -*- coding: utf-8 -*- #
"""*********************************************************************************************"""
#   FileName     [ prepare_tsv.py ]
#   Synopsis     [ preprocess audio speech to generate meta data as tsv files ]
...
def get_preprocess_args():
    ...
    parser = argparse.ArgumentParser(description='preprocess to tsv files for any dataset.')

    parser.add_argument('-i', '--input_data', default='../LibriSpeech/', type=str, help='Path to your LibriSpeech directory', required=False)
    parser.add_argument('-o', '--output_path', default='./', type=str, help='Path to store output', required=False)
    parser.add_argument('-a', '--audio_extension', default='.flac', type=str, help='audio file type (.wav / .flac / .mp3 / etc)', required=False)
    parser.add_argument('-n', '--name', default='tsv_dir', type=str, help='Name of the output directory', required=False)
...
def generate_length(args, tr_set, audio_extension):
    ...
    for i, s in enumerate(tr_set):
        ...
        # Dump data
        df = pd.DataFrame(data={'file_path':[str(args.input_data)]+[fp for fp in sorted_todo], 'length':[""]+list(reversed(sorted(tr_x)))})
        df.to_csv(os.path.join(output_dir, tr_set[i] + '.tsv'), sep="\t", index=False, header=False)
